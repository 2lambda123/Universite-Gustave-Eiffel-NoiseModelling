/**
 * NoiseModelling is a free and open-source tool designed to produce
 * environmental noise maps on very large urban areas. It can be used as a Java
 * library or be controlled through a user friendly web interface.
 *
 * This version is developed by Université Gustave Eiffel and CNRS
 * <http://noise-planet.org/noisemodelling.html>
 * as part of:
 * the Eval-PDU project (ANR-08-VILL-0005) 2008-2011, funded by the Agence
 * Nationale de la Recherche (French) the CENSE project (ANR-16-CE22-0012)
 * 2017-2021, funded by the Agence Nationale de la Recherche (French) the
 * Nature4cities (N4C) project, funded by European Union’s Horizon 2020 research
 * and innovation programme under grant agreement No 730468
 *
 ...
 *
 */
public class LDENPointNoiseMapFactory
    implements PointNoiseMap.PropagationProcessDataFactory,
               PointNoiseMap.IComputeRaysOutFactory, ProfilerThread.Metric {
  ...
  int srid;
  List<String> noiseSource =
      Arrays.asList("ROLLING", "TRACTIONA", "TRACTIONB", "AERODYNAMICA",
                    "AERODYNAMICB", "BRIDGE");
  ...
  public LDENPointNoiseMapFactory(Connection connection,
                                  LDENConfig ldenConfig) {
  ...
  public LDENComputeRaysOut.LdenData getLdenData() { return ldenData; }

  public void insertTrainDirectivity() {
    directionAttributes.clear();
    directionAttributes.put(
        0, new LDENPropagationProcessData.OmnidirectionalDirection());
    int i = 1;
    for (String typeSource : noiseSource) {
      directionAttributes.put(
          i, new RailWayCnossosParameters.RailwayDirectivitySphere(
                 new LineSource(typeSource)));
      i++;
    }
  }
  ...
  public void initialize(Connection connection, PointNoiseMap pointNoiseMap)
    ...
    if (ldenConfig.input_mode == LDENConfig.INPUT_MODE.INPUT_MODE_LW_DEN) {
      // Fetch source fields
      List<String> sourceField = JDBCUtilities.getColumnNames(
          connection, pointNoiseMap.getSourcesTableName());
      this.srid = GeometryTableUtilities.getSRID(
          connection, pointNoiseMap.getSourcesTableName());
      List<Integer> frequencyValues = new ArrayList<>();
      List<Integer> allFrequencyValues = Arrays.asList(
      ...
      if (!period.isEmpty()) {
        for (String fieldName : sourceField) {
          if (fieldName.startsWith(freqField)) {
            int freq =
                Integer.parseInt(fieldName.substring(freqField.length()));
      ...
      for (int freq : frequencyValues) {
        int index = allFrequencyValues.indexOf(freq);
        exactFrequencies.add(
            CnossosPropagationData
                .DEFAULT_FREQUENCIES_EXACT_THIRD_OCTAVE[index]);
        aWeighting.add(
            CnossosPropagationData
                .DEFAULT_FREQUENCIES_A_WEIGHTING_THIRD_OCTAVE[index]);
      }
      if (frequencyValues.isEmpty()) {
        throw new SQLException("Source table " +
                               pointNoiseMap.getSourcesTableName() +
                               " does not contains any frequency bands");
      }
      // Instance of PropagationProcessPathData maybe already set
      for (LDENConfig.TIME_PERIOD timePeriod :
           LDENConfig.TIME_PERIOD.values()) {
        if (pointNoiseMap.getPropagationProcessPathData(timePeriod) == null) {
          PropagationProcessPathData propagationProcessPathData =
              new PropagationProcessPathData(frequencyValues, exactFrequencies,
                                             aWeighting);
          ldenConfig.setPropagationProcessPathData(timePeriod,
                                                   propagationProcessPathData);
          pointNoiseMap.setPropagationProcessPathData(
              timePeriod, propagationProcessPathData);
        } else {
          pointNoiseMap.getPropagationProcessPathData(timePeriod)
              .setFrequencies(frequencyValues);
          pointNoiseMap.getPropagationProcessPathData(timePeriod)
              .setFrequenciesExact(exactFrequencies);
          pointNoiseMap.getPropagationProcessPathData(timePeriod)
              .setFrequenciesAWeighting(aWeighting);
          ldenConfig.setPropagationProcessPathData(
              timePeriod,
              pointNoiseMap.getPropagationProcessPathData(timePeriod));
        }
      }
      ...
      for (LDENConfig.TIME_PERIOD timePeriod :
           LDENConfig.TIME_PERIOD.values()) {
        if (pointNoiseMap.getPropagationProcessPathData(timePeriod) == null) {
          // Traffic flow cnossos frequencies are octave bands from 63 to 8000
          // Hz
          PropagationProcessPathData propagationProcessPathData =
   ...
   */
  public void start() {
    if (ldenConfig.getPropagationProcessPathData(LDENConfig.TIME_PERIOD.DAY) ==
        null) {
      throw new IllegalStateException(
          "start() function must be called after PointNoiseMap initialization call");
    }
  ...
  public LDENPropagationProcessData create(ProfileBuilder builder) {
    LDENPropagationProcessData ldenPropagationProcessData =
        new LDENPropagationProcessData(builder, ldenConfig);
    ldenPropagationProcessData.setDirectionAttributes(directionAttributes);
    return ldenPropagationProcessData;
  }

  @Override
  public IComputeRaysOut create(CnossosPropagationData threadData,
                                PropagationProcessPathData pathDataDay,
    ...
    return new LDENComputeRaysOut(pathDataDay, pathDataEvening, pathDataNight,
                                  (LDENPropagationProcessData)threadData,
                                  ldenData, ldenConfig);
  }
  ...
  private static class TableWriter implements Runnable {
    ...
    public TableWriter(Connection connection, LDENConfig ldenConfig,
                       LDENComputeRaysOut.LdenData ldenData, int srid) {
      this.connection = connection;
      this.sqlFilePath = ldenConfig.sqlOutputFile;
      this.ldenConfig = ldenConfig;
      this.ldenData = ldenData;
      a_weighting = new double[ldenConfig.propagationProcessPathDataDay
                                   .freq_lvl_a_weighting.size()];
      for (int idfreq = 0; idfreq < a_weighting.length; idfreq++) {
        a_weighting[idfreq] =
            ldenConfig.propagationProcessPathDataDay.freq_lvl_a_weighting.get(
                idfreq);
      }
      this.srid = srid;
    }

    void processRaysStack(ConcurrentLinkedDeque<PropagationPath> stack)
        throws SQLException {
      StringBuilder query =
          new StringBuilder("INSERT INTO " + ldenConfig.raysTable +
    ...
    void processStack(
        String tableName,
        ConcurrentLinkedDeque<ComputeRaysOutAttenuation.VerticeSL> stack)
        throws SQLException {
      ...
      if (!ldenConfig.computeLAEQOnly) {
        query.append(", ?".repeat(ldenConfig.propagationProcessPathDataDay
                                      .freq_lvl.size())); // freq value
      ...
      while (!stack.isEmpty()) {
        ...
        if (!ldenConfig.computeLAEQOnly) {
          for (int idfreq = 0;
               idfreq <
               ldenConfig.propagationProcessPathDataDay.freq_lvl.size();
               idfreq++) {
        ...
        // laeq value
        double value =
            wToDba(sumArray(dbaToW(sumArray(row.value, a_weighting))));
        if (!Double.isFinite(value)) {
          value = -99;
        }
        ps.setDouble(parameterIndex++, value);
    ...
    private String forgeCreateTable(String tableName) {
      ...
      if (ldenConfig.computeLAEQOnly) {
        ...
        for (int idfreq = 0;
             idfreq < ldenConfig.propagationProcessPathDataDay.freq_lvl.size();
             idfreq++) {
          sb.append(", HZ");
          sb.append(
              ldenConfig.propagationProcessPathDataDay.freq_lvl.get(idfreq));
          sb.append(" REAL");
        }
    ...
    public void init() throws SQLException, IOException {
      if (ldenConfig.getExportRaysMethod() ==
          LDENConfig.ExportRaysMethods.TO_RAYS_TABLE) {
        if (ldenConfig.dropResultsTable) {
          String q =
              String.format("DROP TABLE IF EXISTS %s;", ldenConfig.raysTable);
          processQuery(q);
        }
        StringBuilder sb = new StringBuilder(
            "CREATE TABLE IF NOT EXISTS " + ldenConfig.raysTable +
      ...
      if (ldenConfig.computeLDay) {
        if (ldenConfig.dropResultsTable) {
          String q =
              String.format("DROP TABLE IF EXISTS %s;", ldenConfig.lDayTable);
          processQuery(q);
        }
      ...
      if (ldenConfig.computeLEvening) {
        if (ldenConfig.dropResultsTable) {
          String q = String.format("DROP TABLE IF EXISTS %s;",
      ...
      if (ldenConfig.computeLNight) {
        if (ldenConfig.dropResultsTable) {
          String q =
              String.format("DROP TABLE IF EXISTS %s;", ldenConfig.lNightTable);
          processQuery(q);
        }
      ...
      if (ldenConfig.computeLDEN) {
        if (ldenConfig.dropResultsTable) {
          String q =
              String.format("DROP TABLE IF EXISTS %s;", ldenConfig.lDenTable);
          processQuery(q);
        }
    ...
    OutputStreamWriter getStream() throws IOException {
      if (ldenConfig.sqlOutputFileCompression) {
        return new OutputStreamWriter(new GZIPOutputStream(
            new FileOutputStream(sqlFilePath), WRITER_CACHE));
      } else {
